...
- `Game.Board` is a `Board` (`[][]int`, slice → reference type), so `copyGame` must call `Board.Clone()` to deep-copy it; a bare shallow copy would share backing rows and reintroduce a read/write race. Guarded by `service_concurrent_test.go` + `go test -race`.
- `Board` owns move rules: `Set(x,y,mark)` validates bounds/occupancy itself and returns `ErrOutOfBounds`/`ErrCellOccupied` (those errs live in `board.go`, not `game.go`); win is `HasLine` (full row/col/diag of N), draw is `IsFull`. Board size is per-game via `NewBoard(size)`; `DefaultSize`=3. JSON wire stays a 2D int array, no custom marshalling.
